...
-- BuilderSans is the current Roblox UI face. SourceSans is the 2010 face and drops ▾ / emoji.
local FONT_TITLE = Enum.Font.BuilderSansBold
local FONT_BODY = Enum.Font.BuilderSansMedium
local FONT_VALUE = Enum.Font.Code
local FONT_ICON = Enum.Font.GothamBold
local FONT_DROP = Enum.Font.BuilderSans
local CHEVRON_DN = "v"
local CHEVRON_UP = "^"
...
local function createStateCell(parent, enabled, opts)
    ...
    local w = h
    if mode == "switch" then
        w = math.floor(h * 1.8 + 0.5)
    end
    local cell = create("Frame", {
                BackgroundColor3 = enabled and CurrentTheme.accent or CurrentTheme.hover,
    ...
    local knob = create("Frame", {
        BackgroundColor3 = enabled and CurrentTheme.text or CurrentTheme.textDim,
        BorderSizePixel = 0,
    ...
    local function layout(animate)
        ...
        if mode == "switch" then
            knob.Visible = true
            tick.Visible = false
            knob.BackgroundColor3 = state and t.text or t.textDim
            local pos = state and UDim2.fromOffset(w - knobH - knobM, knobM) or UDim2.fromOffset(knobM, knobM)
            if animate == false then
                cell.BackgroundColor3 = state and t.accent or t.hover
                cell.BackgroundTransparency = 0
                knob.Position = pos
            else
                tween(cell, { BackgroundColor3 = state and t.accent or t.hover }, 0.10)
            ...
            if animate == false then
                ...
                tween(cell, {
                    BackgroundColor3 = state and t.accent or t.hover,
                    BackgroundTransparency = 0,
                }, 0.10)
            end
        end
...
function UILib:CreateWindow(cfg)
    ...
    local function isCompactViewport()
        ...
        return UIS.TouchEnabled or w < 700
    end

    local function getSearchBarH()
        if isCompactViewport() then
            return 44
        end
        return 28
    end
    ...
    local function getSwitchW()
        return math.floor(getStateCellSize() * 1.9 + 0.5)
    end

    -- Always follow viewport; AbsoluteSize may be 0 at inject (relayout fixes later)
    local function getPanelWidth()
        local w = gui.AbsoluteSize.X
        if w < 1 then
            return scaled(Config.panelWidth)
        end
        return math.clamp(math.floor(w * 0.18), 168, 210)
    end

    -- Left-column spawn / reflow for game panels (ignores Settings/Misc/Graphics)
    local TOP_MARGIN = 10

    local function nextGamePanelSlot(panelWidth, excludePanel)
        local compact = isCompactViewport()
        local vp = gui.AbsoluteSize
        local gap = compact and 8 or 15
        local margin = compact and 8 or 15
        local startY = TOP_MARGIN
        if showSearchBar and searchBarPosition == "Top" then
            startY = TOP_MARGIN + getSearchBarH() + 8
        end
        local headerH = scaled(Config.headerHeight)
        local rightLimit = math.max(margin + panelWidth, vp.X - panelWidth - 8)
        if vp.X < 1 then
            rightLimit = 10000
        end
        local x = margin
        local y = startY
        local rowMaxH = headerH
        for _, p in ipairs(windowPanels) do
            if p._rightStack or not p.Instance or p == excludePanel then
                -- skip
            else
                ...
                if x + panelWidth > rightLimit and x > margin then
                    x = margin
                    y = y + rowMaxH + gap
                    rowMaxH = headerH
                end
        ...
        if vp.X >= 1 then
            x = math.clamp(x, 0, math.max(0, vp.X - panelWidth))
            y = math.clamp(y, 0, math.max(0, vp.Y - headerH))
        end
        return UDim2.fromOffset(math.floor(x + 0.5), math.floor(y + 0.5))
    end

    local function reflowGamePanels(pw)
        local compact = isCompactViewport()
        local vp = gui.AbsoluteSize
        if vp.X < 1 then
            return
        ...
        local headerH = scaled(Config.headerHeight)
        local rightLimit = math.max(margin + pw, vp.X - pw - 8)
        local x = margin
        local y = startY
        local rowMaxH = headerH
        for _, p in ipairs(windowPanels) do
            if p._rightStack or p._userMoved or not p.Instance then
                -- skip right stack and user-dragged panels
            else
                local inst = p.Instance
                local h = inst.AbsoluteSize.Y
                if h < 1 then
                ...
                if x + pw > rightLimit and x > margin then
                    x = margin
                    y = y + rowMaxH + gap
                    rowMaxH = headerH
                end
    ...
    local function getModuleH()
        local h = scaled(Config.moduleHeight)
        if isCompactViewport() then
    ...
    local function getSettingH()
        local h = math.floor(Config.settingHeight * Config.uiScale + 0.5)
        if isCompactViewport() then
            return math.max(h, 44)
        end
        return h
    ...
    if gui.AbsoluteSize.X > 0 and gui.AbsoluteSize.X < 400 then
        Config.uiScale = 0.9
    ...
    local function updateNotifPosition()
        if notifyPosition == "TopRight" then
            ...
            notifContainer.Position = UDim2.new(1, -15, 0, TOP_MARGIN)
        elseif notifyPosition == "TopLeft" then
            notifContainer.AnchorPoint = Vector2.new(0, 0)
            notifContainer.Position = UDim2.new(0, 15, 0, TOP_MARGIN)
    ...
    local function layoutSearchBar()
        ...
        searchBarFrame.Size = UDim2.fromOffset(math.min(300, math.max(80, vpX - 24)), searchH)
        if searchBarPosition == "Bottom" then
            searchBarFrame.AnchorPoint = Vector2.new(0.5, 1)
            searchBarFrame.Position = UDim2.new(0.5, 0, 1, -TOP_MARGIN)
        else
            searchBarFrame.AnchorPoint = Vector2.new(0.5, 0)
            searchBarFrame.Position = UDim2.new(0.5, 0, 0, TOP_MARGIN)
        end
        if searchResults then
            if searchBarPosition == "Bottom" then
                searchResults.AnchorPoint = Vector2.new(0, 1)
                searchResults.Position = UDim2.fromOffset(0, -4)
            else
                searchResults.AnchorPoint = Vector2.new(0, 0)
                searchResults.Position = UDim2.fromOffset(0, searchH + 1)
            end
        end
    end
    ...
    local function showTooltip(text, target, fromFocus)
        ...
        local w = math.min(280, math.max(48, bounds.X + 16))
    ...
    local function setUIVisible(visible)
        ...
        if searchBarFrame then
            searchBarFrame.Visible = visible and showSearchBar
        end
        hint.Visible = (not visible) and SavedSettings.showOpenHint ~= false
        ...
        for _, p in ipairs(windowPanels) do
            if p.Instance then
                ...
                elseif visible then
                    ...
                    p.Instance.Visible = p.Pinned == true
                end
            end
        end
        if panelNavFrame then
            panelNavFrame.Visible = visible
        end
    ...
    function window:Notify(text, duration, options)
        ...
        local vpMax = math.max(120, gui.AbsoluteSize.X - 30)
        maxW = math.min(maxW, vpMax)
    ...
    function window:ShowChangelog()
        ...
        local vp = gui.AbsoluteSize
        local clW = math.min(750, math.max(160, vp.X - 16))
        local clH = math.min(650, math.max(160, vp.Y - 16))
        changelogFrame = create("Frame", {
            Name = generateRandomName(8),
            BackgroundColor3 = theme.bg,
            BorderSizePixel = 0,
            Size = UDim2.fromOffset(clW, clH),
            AnchorPoint = Vector2.new(0.5, 0.5),
            Position = UDim2.fromScale(0.5, 0.5),
            ZIndex = 800,
            Parent = gui
    ...
    function window:SetDebugMode(enabled)
        ...
        if Config.debugMode then
            ...
            debugPanel.Instance.Visible = uiVisible
        else
        ...
        local row = panelNavRows[debugPanel]
        if row then
            row.Visible = Config.debugMode
        end
    end
    ...
    function window:SetArrayListPosition(pos)
        ...
        if pos == "Left" then
            ...
            arrayList.Position = UDim2.new(0, 10, 0, TOP_MARGIN)
            ...
            arrayList.Position = UDim2.new(1, -10, 0, TOP_MARGIN)
            local layout = arrayList:FindFirstChildOfClass("UIListLayout")
    ...
    local function refreshPanelNavTheme()
        ...
        panelNavFrame.BackgroundColor3 = t.panel
        panelNavTitle.TextColor3 = t.textDim
        local st = panelNavFrame:FindFirstChildOfClass("UIStroke")
        if st then
            st.Color = OUTLINE
        end
        for _, p in ipairs(windowPanels) do
            ...
            if row then
                paintNavRow(row, p._manuallyHidden == true)
                updateHoverColors(row, t.bg, t.hover)
            end
        end
    end
    ...
    local function centerPanelOnScreen(p)
        ...
        local vp = gui.AbsoluteSize
        local scale = Config.uiScale
        if scale < 0.01 then
            scale = 1
        end
        local visW = inst.AbsoluteSize.X
        if visW < 1 then
            visW = (p._panelWidth or getPanelWidth()) * scale
        end
        local visH = inst.AbsoluteSize.Y
        if visH < 1 then
            visH = scaled(Config.headerHeight) * scale
        end
        local x = (vp.X * 0.5 - visW * 0.5) / scale
        local y = (vp.Y * 0.5 - visH * 0.5) / scale
        x, y = clampOffset(x, y, visW, visH, scale)
        inst.Position = UDim2.fromOffset(math.floor(x + 0.5), math.floor(y + 0.5))
    end

    local function registerPanelNav(p)
        if not p or not p.Instance or panelNavRows[p] then
            return
        end
        local rowH = isCompactViewport() and 44 or 22
        local row = create("TextButton", {
            BackgroundColor3 = CurrentTheme.bg,
            BorderSizePixel = 0,
            Size = UDim2.fromOffset(132, rowH),
            Text = "",
            AutoButtonColor = false,
            ZIndex = 482,
            Parent = panelNavFrame,
        ...
        local cell = createStateCell(row, true, {
            mode = "mark",
            size = 10,
        ...
        navCells[row] = cell
        trackHover(row, CurrentTheme.bg, CurrentTheme.hover)
        paintNavRow(row, false)
        panelNavRows[p] = row
        bindActivate(row, function()
            if p.toggleFromNavigator then
                p:toggleFromNavigator()
            end
        end)
    end
    ...
    local function restoreLiftedPanel()
        if focusedFieldRestore then
            local inst = focusedFieldRestore.inst
    ...
    local function beginFieldLift(box)
        ...
        local panelInst
        for _, p in ipairs(windowPanels) do
            if p.Instance and box:IsDescendantOf(p.Instance) then
                panelInst = p.Instance
                break
            end
        end
        if not panelInst then
            return
        end
        focusedFieldRestore = { inst = panelInst, pos = panelInst.Position }
        local kbY = 0
        pcall(function()
        ...
        local limit = vp.Y - kbY - 8
        if bottom > limit then
            local delta = bottom - limit
            local x, y = offsetFromUDim(panelInst.Position)
            panelInst.Position = UDim2.fromOffset(x, y - delta / math.max(Config.uiScale, 0.01))
        end
    end
    ...
    function window:AddPanel(name, position)
        ...
        if not position then
            position = nextGamePanelSlot(panelWidth)
        end
        ...
        local panel = create("Frame", {
            ...
            BackgroundColor3 = theme.panel,
            BorderSizePixel = 0,
            Size = UDim2.fromOffset(panelWidth, headerH),
            Position = position,
            AutomaticSize = Enum.AutomaticSize.Y,
            Active = true,
        ...
        makeRounded(body, Config.cornerRadius)
        ...
        local collapseBtn = create("TextButton", {
            ...
            Position = UDim2.new(1, compact and -(btnSize + 4) or -28, 0.5, -btnSize / 2),
            Text = CHEVRON_DN,
            TextColor3 = theme.textDim,
            Font = FONT_ICON,
            TextSize = scaled(14),
        ...
        local pinBtn = create("TextButton", {
            BackgroundTransparency = 1,
            Size = UDim2.fromOffset(btnSize, btnSize),
            Position = UDim2.new(1, compact and -(btnSize * 2 + 4) or -52, 0.5, -btnSize / 2),
            Text = "",
            AutoButtonColor = false,
            Active = true,
            ZIndex = 3,
        ...
        makeRounded(pinBtn, Config.controlRadius)
        ...
        local function clampPanelToViewport()
            local visW = panel.AbsoluteSize.X
            if visW < 1 then
                visW = panelWidth * Config.uiScale
            end
            local visH = header.AbsoluteSize.Y
            if visH < 1 then
                visH = scaled(Config.headerHeight) * Config.uiScale
            end
            local x, y = offsetFromUDim(panel.Position)
            x, y = clampOffset(x, y, visW, visH, Config.uiScale)
            panel.Position = UDim2.fromOffset(math.floor(x + 0.5), math.floor(y + 0.5))
        end
        ...
        bindActivate(collapseBtn, function()
            ...
            if collapsed then
                ...
                setTreeSelectable(content, false)
                ...
                tween(panel, { Size = UDim2.fromOffset(panelWidth, scaled(Config.headerHeight)) }, 0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
            else
                contentHeight = measureContentHeight()
                local targetHeight = scaled(Config.headerHeight) + contentHeight
                tween(panel, { Size = UDim2.fromOffset(panelWidth, targetHeight) }, 0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
                task.delay(0.08, function()
                    content.Visible = true
                    setTreeSelectable(content, true)
        ...
        header.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                local mouse = UIS:GetMouseLocation()
                if guiContains(pinBtn, mouse) or guiContains(collapseBtn, mouse) then
                    return
                end
                local top = topmostPanelAtMouse()
                if not top or top.Instance ~= panel then
                    return
                end
                dragging = true
                dragOwner = panel
                if panelObj then
                    panelObj._userMoved = true
                end
                panelFrontZ = panelFrontZ + 1
                panel.ZIndex = panelFrontZ
        ...
        local dragConn = UIS.InputChanged:Connect(function(input)
            if not dragging or dragOwner ~= panel then return end
            if input.UserInputType ~= Enum.UserInputType.MouseMovement and input.UserInputType ~= Enum.UserInputType.Touch then return end
            ...
            newX, newY = clampOffset(newX, newY, visW, visH, Config.uiScale)
        ...
        function panelObj:SetPinned(v)
            ...
            updatePinVisual()
            if not uiVisible then
                panel.Visible = panelObj.Pinned == true and not panelObj._manuallyHidden
            elseif panelObj._manuallyHidden then
                panel.Visible = false
            end
        end
        ...
        function panelObj:AddModule(cfg)
            ...
            local expandW = hasSettings and (isCompactViewport() and 28 or 20) or 0
            if isToggle then
            ...
            if hasSettings then
                settingsContainer = create("Frame", {
                    ...
                    BorderSizePixel = 0,
                    Size = UDim2.new(1, 0, 0, 0),
                    Position = UDim2.fromOffset(0, moduleH),
                    ClipsDescendants = true,
                    Visible = false,
                    AutomaticSize = Enum.AutomaticSize.None,
                    Parent = moduleHolder
        ...
        function panelObj:_addSetting(container, setting)
            ...
            elseif settingType == "slider" then
                local row = create("Frame", {
                    BackgroundTransparency = 1,
                    Size = UDim2.new(1, 0, 0, getSettingH() + 12),
                    Parent = container
                })
            ...
            elseif settingType == "dropdown" then
                ...
                local function closeDrop()
                    ...
                    tween(row, { Size = UDim2.new(1, 0, 0, dropLabelH + dropH + 4) }, 0.10, Enum.EasingStyle.Quad)
                    task.delay(0.10, function()
                ...
                dropBtn.Activated:Connect(function()
                    ...
                    open = true
                    dropStroke.Color = CurrentTheme.accent
                    dropArrow.Text = CHEVRON_UP
                    openDropdownCloser = closeDrop
                    openDropdownNav = {
                        move = function(delta)
                            if #optionButtons == 0 then
                                return
                            end
                            dropOptSel = dropOptSel + delta
                            if dropOptSel < 1 then
                                dropOptSel = #optionButtons
                            end
                            if dropOptSel > #optionButtons then
                                dropOptSel = 1
                            end
                            updateOptionVisuals()
                            local data = optionButtons[dropOptSel]
                            if data then
                                local sel = data.click
                                if not sel and data.btn and data.btn:IsA("GuiButton") then
                                    sel = data.btn
                                end
                                if sel then
                                    pcall(function()
                        ...
                        activate = function()
                            ...
                            if isMultiple then
                                ...
                                applySingle(data.opt)
                            end
                        end,
                    }
                    updateOptionVisuals()
                    
                    local optionHeight = getFieldH()
                    local maxHeight = optionHeight * maxVisibleOptions
                    local listH = math.min(math.max(#options, 1) * optionHeight, maxHeight)
                    local vp = gui.AbsoluteSize
                    local btnAbs = dropBtn.AbsolutePosition
                    local btnSz = dropBtn.AbsoluteSize
                    local spaceBelow = vp.Y - (btnAbs.Y + btnSz.Y) - 8
                    local spaceAbove = btnAbs.Y - 8
                    local remain = math.max(spaceBelow, spaceAbove, 48)
                    listH = math.min(listH, remain)
                    local openUp = spaceBelow < listH and spaceAbove > spaceBelow
                    if openUp then
                        dropList.Position = UDim2.new(0, 0, 0, dropBtn.Position.Y.Offset - listH)
                    else
                        dropList.Position = UDim2.new(0, 0, 0, dropLabelH + dropH + 2)
                    end
                    ...
                    if not openUp then
                        tween(row, { Size = UDim2.new(1, 0, 0, dropLabelH + dropH + 4 + listH + 4) }, 0.12, Enum.EasingStyle.Quad)
                    end
                end)
    ...
    local function updateGraphicsPosition()
        ...
        local g = graphicsPanel.Instance
        ...
        g.Position = UDim2.new(
            ...
            m.Position.X.Offset,
            m.Position.Y.Scale,
            m.Position.Y.Offset + m.AbsoluteSize.Y + 8
        )
        if graphicsPanel._clamp then
            graphicsPanel._clamp()
        end
    end
    ...
    local function updateMiscPosition()
        ...
        if settingsPanel._clamp then
            settingsPanel._clamp()
        end
        ...
        if miscPanel._clamp then
            miscPanel._clamp()
        end
        updateGraphicsPosition()
    end
    settingsPanel.Instance:GetPropertyChangedSignal("AbsoluteSize"):Connect(updateMiscPosition)
    miscPanel.Instance:GetPropertyChangedSignal("AbsoluteSize"):Connect(updateGraphicsPosition)
    task.defer(updateMiscPosition)
    task.defer(updateGraphicsPosition)

    local function relayoutFloatingUi()
        local vp = gui.AbsoluteSize
        local pw = getPanelWidth()
        layoutSearchBar()
        notifContainer.Size = UDim2.fromOffset(math.min(500, math.max(120, vp.X - 30)), 600)
        if changelogFrame then
            local clW = math.min(changelogFrame.AbsoluteSize.X, math.max(160, vp.X - 16))
            local clH = math.min(changelogFrame.AbsoluteSize.Y, math.max(160, vp.Y - 16))
            changelogFrame.Size = UDim2.fromOffset(clW, clH)
        end
        clampToggleBtn()
        layoutHintAndMobileOpen()
        settingsPanel.Instance.Position = getRightStackPosition()
        for _, p in ipairs(windowPanels) do
            p._panelWidth = pw
            if p.Instance then
                local keepY = p.Instance.Size.Y.Offset
                if keepY < 1 then
                    keepY = scaled(Config.headerHeight)
                end
                p.Instance.Size = UDim2.fromOffset(pw, keepY)
            end
        end
        reflowGamePanels(pw)
        for _, p in ipairs(windowPanels) do
            if not p._rightStack and p._clamp and p._userMoved then
                p._clamp()
            end
        end
        updateMiscPosition()
        ...
        if freecamMobileHud then
            ...
            local hx = math.clamp(8, 0, math.max(0, vp.X - hudW))
            local hy = math.clamp(vp.Y - hudH - 8, 0, math.max(0, vp.Y - hudH))
    ...
    if showSplash then
        ...
        task.defer(function()
            ...
            if panelNavFrame then panelNavFrame.Visible = uiVisible end
        end)
    end
